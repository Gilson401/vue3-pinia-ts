...
const BASE_PREFIX = import.meta.env.VITE_API_BASEURL_POKEMON;
...
const service = {
  ...
  getEspecies<T = any>(): Promise<T> {
    return axiosInstance.get('pokemon-species', { params: { limit: 1000 } });
  },

  getPokemonByName<T = any>(url: string): Promise<T> {
